...
import smtplib
from user_task_classes import ForUser
...
def CreateUser():
    ...
    mycursor.execute("CREATE TABLE user (user_id INT, user_name VARCHAR(20),phone VARCHAR(20), role VARCHAR(20), "
                     "dob VARCHAR(20), created_on  VARCHAR(20), modified_on VARCHAR(20), Email VARCHAR(50))")
    mydb.commit()
...
def InsertIntoUser(user_task_classes):
    mydb = ConnectToDB()
    mycursor = mydb.cursor()
    sql = "INSERT INTO user (user_id, user_name, phone, role, dob, created_on, modified_on , Email) VALUES (%s ,%s, %s, %s, %s, %s, %s, %s)"
    val = (user_task_classes.user_id, user_task_classes.user_name, user_task_classes.phone, user_task_classes.role, user_task_classes.dob,
               user_task_classes.created_on, user_task_classes.modified_on , user_task_classes.Email)
...
def AssignTask(user_id,task_id):
    ...
    se.login("airbuddy.weirdo@gmail.com", "")
    se.sendmail("airbuddy.weirdo@gmail.com", SendEmail(user_id) , "You are onboarded on project!!")
    se.quit()
